...
typetitle = function() {
            ...
            const deletingSpeed = 50;   // Speed of deleting each letter
            ...
            function typeLoop() {
                ...
                if (isDeleting) {
                    // Remove a character using jQuery .text()
                    $textElement.text(currentPhrase.substring(0, currentCharIndex - 1));
                    currentCharIndex--;
                } else {
                    // Add a character using jQuery .text()
                    $textElement.text(currentPhrase.substring(0, currentCharIndex + 1));
                    currentCharIndex++;
                }

                // Determine typing speed based on whether it's deleting or typing
                let timeoutSpeed = isDeleting ? deletingSpeed : typingSpeed;

                // If word is completely typed out
                if (!isDeleting && currentCharIndex === currentPhrase.length) {
                    timeoutSpeed = pauseEnd; // Pause at the end of the word
                    isDeleting = true;
                } 
                // If word is completely deleted
                else if (isDeleting && currentCharIndex === 0) {
                    isDeleting = false;
                    // Move to the next phrase and loop back using modulo
                    currentPhraseIndex = (currentPhraseIndex + 1) % phrases.length;
                    timeoutSpeed = pauseStart; // Pause before typing the next word
                }
